...
class TestCLIIntegration:
    ...
    def test_temporary_corpus_key_auto_generation(self):
        ...
        # Create a simple mock environment structure
        with tempfile.TemporaryDirectory() as tmpdir:
            corpus_keys_dir = Path(tmpdir) / "environments" / "staging" / "corpus_keys" / "TEST_BOT"
            corpus_keys_dir.mkdir(parents=True)
            ...
            assert "Successfully transformed 1 objects for temporary deployment" in result.stderr
    ...
    def test_url_template_generation_output(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run([
                ...
                "--lambda-function-name", "test-function",
                "--project-root-path", tmpdir,
                "--temporary-corpus-key", "TEMP_TEST123",
            ...
            assert "Test Bot URL: https://app.staging.poemai.ch/ui/town_bot/app/TEMP_TEST123/" in result.stderr
            # Check for GitHub Actions notice
            assert "::notice title=Test Bot URL::🔗 https://app.staging.poemai.ch/ui/town_bot/app/TEMP_TEST123/" in result.stdout
    ...
    def test_temporary_corpus_key_validation_failure(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run([
                ...
                "--lambda-function-name", "test-function",
                "--project-root-path", tmpdir,
                "--temporary-corpus-key", "TEMP_SUCCESS"
            ], capture_output=True, text=True, env=env)
            ...
            assert 'Successfully transformed 2 objects for temporary deployment' in result.stderr
