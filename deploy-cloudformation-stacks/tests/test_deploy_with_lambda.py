...
def test_deploy_2(tmpdir):
    ...
    with patch("deploy_with_lambda_call.boto3", name="boto3_mock") as boto3_mock:
...
def test_resolve_version_with_hash_support():
    ...
    repo_versions = {"poemAI-ch/poemai-lambdas": "abcdef1234567890"}
    result = resolve_version_with_hash_support(
        "poemAI-ch/poemai-lambdas", repo_versions, "SomeVersion"
    )
    ...
    result = resolve_version_with_hash_support(
        "poemAI-ch/poemai-lambdas#bot_admin", repo_versions, "BotAdminLambdaVersion"
    )
    assert result == "1234567890abcdef"

    result = resolve_version_with_hash_support(
        "poemAI-ch/poemai-lambdas#assistant_api",
        repo_versions,
        "AssistantAPILambdaVersion",
    )
    ...
    result = resolve_version_with_hash_support(
        "poemAI-ch/poemai-lambdas#nonexistent_lambda",
        repo_versions,
        "NonExistentVersion",
    )
    ...
    result = resolve_version_with_hash_support(
        "poemAI-ch/nonexistent-repo", repo_versions, "SomeVersion"
    )
    assert result is None
...
def test_stack_filtering_single_stack(tmpdir, caplog):
    ...
    for stack_name in ["stack_one", "stack_two", "stack_three"]:
        stack_template = {
            ...
            "Resources": {
                "TestResource": {
                    "Type": "AWS::S3::Bucket",
                    "Properties": {"BucketName": f"{stack_name}-bucket"},
                }
            },
        }
    ...
    summary_logs = [
        ...
        if "Summary: Successfully processed" in record.message
    ]
    ...
    stack_logs = [
        record.message
        for record in caplog.records
        if "✓" in record.message and "stack-two-development" in record.message
    ]
    ...
    other_stack_logs = [
        ...
        and (
            "stack-one-development" in record.message
            or "stack-three-development" in record.message
        )
    ]
...
def test_stack_filtering_all_stacks(tmpdir, caplog):
    ...
    with caplog.at_level(logging.INFO):
        do_dump(
            config, stacks_file.as_posix(), environment, verbose=False, stack_name=None
        )
    ...
    alpha_logs = [
        record.message
        for record in caplog.records
        if "✓" in record.message and "stack-alpha-development" in record.message
    ]
    beta_logs = [
        record.message
        for record in caplog.records
        if "✓" in record.message and "stack-beta-development" in record.message
    ]
...
def test_stack_filtering_nonexistent_stack(tmpdir, caplog):
    ...
    stack_template = {
        ...
        "Resources": {
            "TestResource": {
                "Type": "AWS::S3::Bucket",
                "Properties": {"BucketName": "real-stack-bucket"},
            }
        },
    }
...
def test_stack_filtering_both_name_formats(tmpdir, caplog):
    ...
    with caplog.at_level(logging.INFO):
        do_dump(
            ...
            environment,
            verbose=False,
            stack_name="test-stack-beta-development",
        )
...
def test_override_globals_file_functionality(tmpdir, caplog):
    ...
    stack_template = {
        ...
        "Resources": {
            "TestResource": {
                "Type": "AWS::S3::Bucket",
                "Properties": {"BucketName": {"Fn::Sub": "test-bucket-${Version}"}},
            }
        },
    }
    ...
    with patch("deploy_with_lambda_call.load_globals_from_file") as mock_load:
        ...
        with caplog.at_level(logging.DEBUG):
            ...
            message_generations, dependency_graph = prepare_messages(
                config, stacks_file.as_posix()
            )
        ...
        assert (
            messages[0]["message"]["parameters"]["Version"] == "overridden-version-456"
        )
...
def test_override_globals_file_functionality(tmpdir, caplog):
    ...
    from deploy_with_lambda_call import load_config, prepare_messages
        ...
        "stacks": [
            {
                ...
                "parameters": {
                    "Environment": {"$ref": "Environment"},
                    "LambdaS3Key": {
                        "$sub": "lambdas/test_lambda/${TestLambdaVersion}/test_lambda.zip"
                    },
    ...
    versions_data = {"versions": {"poemAI-ch/test-repo#test_lambda": "abcdef123456"}}
    ...
    config["repo_versions_file"] = (
        "../versions.yaml"  # Relative to the stacks.yaml file
    )
    ...
    test_message = next(
        msg
        for msg in messages
        if msg["message"]["stack_name"] == "test-stack-development"
    )
    assert (
        "lambdas/test_lambda/abcdef123456/test_lambda.zip"
        in test_message["message"]["parameters"]["LambdaS3Key"]
    )
    ...
    test_args = [
        "deploy_with_lambda_call.py",
        "dump",
        str(stacks_file),
        "--override-globals-file",
        str(override_globals_file),
    ]
...
def test_deploy_fails_with_missing_dependencies(tmpdir):
    ...
    with patch("deploy_with_lambda_call.boto3") as boto3_mock:
        ...
        with pytest.raises(ValueError) as exc_info:
            deploy(
                "test-lambda-function", config, stacks_file.as_posix(), stack_name=None
            )

        # The error should mention failed dependencies
        error_message = str(exc_info.value)
        assert (
            "Cannot deploy" in error_message
            and "dependencies have not been successfully deployed" in error_message
        )
...
def test_empty_template_file_handling(tmpdir):
    ...
    assert (
        "empty_template.yaml" in error_message
        or "comments_only_template.yaml" in error_message
        or "whitespace_only_template.yaml" in error_message
    )
    ...
    stacks = {
        ...
        "stacks": [
            ...
            },
            {
                ...
                "dependencies": [
                    "stack-a"
                ],  # Disabled depends on disabled - should be OK
            ...
            },
            {
                ...
                "dependencies": [
                    "stack-b"
                ],  # Enabled depends on disabled - should be ERROR
...
def test_fully_disabled_chain_is_allowed(tmpdir):
    ...
    try:
        ...
        assert len(message_generations) == 0 or all(
            len(generation) == 0 for generation in message_generations
        )
    except ValueError:
        pytest.fail(
            "Should not raise ValueError when all stacks in dependency chain are disabled"
        )
